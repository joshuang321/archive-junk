...
#define stack_types_are_defined 1

#define DECLARE_STACK_TYPE( type )                         \
typedef struct type##_stack_t                              \
{                                                          \
    type *buffer;                                          \
    int front;                                             \  
    int back;                                              \
    ...
    while ( find_operands != NULL )
    {
        ...
        while(true)
        {
            if ( !char_stack_empty( &temp_operands ) )
            {
                ...
                find_operands[0] == '^' )
                {
                    char_stack_push( &temp_operands, find_operands[0] );
                    break;
                }
                ...
                else if ( find_operands[0] == ')' )
                {
                    ...
                    {
                        strncat( postfixStr, char_stack_pop( &temp_operands ), 1 );
                        strncat( postfixStr, " ", 1);
                    }
...
double evaluate_postfix(const char *postfixStr)
{
    ...
    while ( find_delims != NULL )
    {
        ...
        switch ( buf[0] )
        {
            ...
            tempval = double_stack_pop( &temp_doubles )[0] * tempval;
...
char* remove_whitespace(char* unmodinfix )
{
    ...
    {
        memmove( find_whitespace, find_whitespace + 1, strlen( find_whitespace + 1 ) + 1);
